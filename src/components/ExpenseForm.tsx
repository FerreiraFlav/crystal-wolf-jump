...
export const ExpenseForm: React.FC<ExpenseFormProps> = ({ onAddExpense }) => {
  ...
  return (
    <Card className="bg-white dark:bg-slate-900 border-slate-200/80 dark:border-slate-800 shadow-sm rounded-2xl overflow-hidden transition-colors">
      <CardHeader className="bg-slate-50/70 dark:bg-slate-800/40 border-b border-slate-100 dark:border-slate-800 pb-3.5 transition-colors">
        <CardTitle className="text-base font-bold text-slate-800 dark:text-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className={`p-1.5 rounded-lg ${type === 'expense' ? 'bg-red-100 dark:bg-red-950/60 text-red-700 dark:text-red-400' : 'bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400'}`}>
          ...
          <div className="flex p-0.5 bg-slate-200/80 dark:bg-slate-800 rounded-xl border border-transparent dark:border-slate-700/60">
            <button
              type="button"
              onClick={() => handleTypeSwitch('expense')}
              className={`px-3 py-1 rounded-lg text-xs font-bold transition-all flex items-center gap-1 ${
                type === 'expense' ? 'bg-red-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg text-xs font-bold transition-all flex items-center gap-1 ${
                type === 'income' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <span className="absolute left-3 top-2.5 text-slate-500 dark:text-slate-400 font-bold text-sm">
                {currencySymbol}
              </span>
              <Input
                id="amount"
                ...
                className="pl-9 font-bold text-slate-800 dark:text-white text-lg rounded-xl border-slate-200 dark:border-slate-700 dark:bg-slate-800/60 focus:border-emerald-500 focus:ring-emerald-500"
                required
              />
            </div>
          </div>
          ...
          {/* Descrição */}
          <div className="space-y-1.5">
            <Label htmlFor="description" className="text-xs font-semibold text-slate-700 dark:text-slate-300">{t('description')}</Label>
            <div className="relative">
          ...
          {/* Categoria */}
          <div className="space-y-1.5">
            <Label htmlFor="category" className="text-xs font-semibold text-slate-700 dark:text-slate-300">{t('category')}</Label>
            <div className="relative">
              ...
              <select
                ...
                className="w-full pl-9 pr-4 py-2 bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-medium text-slate-700 dark:text-slate-200 focus:border-emerald-500 focus:ring-emerald-500 outline-none transition-colors cursor-pointer"
              >
                {categories.map(cat => (
          ...
          {/* Data */}
          <div className="space-y-1.5">
            <Label htmlFor="date" className="text-xs font-semibold text-slate-700 dark:text-slate-300">{t('date')}</Label>
            <div className="relative">
              ...
              <Input
                ...
                className="pl-9 rounded-xl border-slate-200 dark:border-slate-700 dark:bg-slate-800/60 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500 [color-scheme:light] dark:[color-scheme:dark]"
                required
              />
            </div>
          </div>
